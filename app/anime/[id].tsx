...
import { AnimeDetail, fetchAnimeDetail } from "@/lib/anilist";
import { supabase } from "@/lib/supabase";
import { useLocalSearchParams, useRouter } from "expo-router";
import { ChevronLeft, PlayCircle, Plus, Trash2 } from "lucide-react-native";
...
export default function AnimeDetailScreen() {
  ...
  const updateAnimeStatus = async (
    ...
    await supabase.from("watch_list").update({ status }).eq("id", currentId);
  };
    ...
    try {
      if (newValue) {
        ...
        await supabase.from("watched_episodes").insert({
          watch_list_id: currentWatchListId,
          episode_number: episodeNumber,
        });
        setWatchedEpisodes((prev) => [...prev, episodeNumber]);
        await updateAnimeStatus(currentWatchListId, [
          ...watchedEpisodes,
          episodeNumber,
        ]);
      } else {
            ...
            <View style={styles.infoSection}>
              <View style={styles.coverWrapper}>
                <Image
                  ...
                  style={[styles.cover, { borderColor: colors.background }]}
                />
              </View>
